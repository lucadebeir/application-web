...
utiliserIngredients.delete(
  "/:idRecette/ingredient/:idIngredient/delete",
  (req, res) => {
    UtiliserIngredients.destroy({
      ...
      .then(() => {
        res.json({ success: "Ingredient deleted!" });
      })
      .catch((err) => {
        res.send("error: " + err);
      });
...
utiliserIngredients.post(
  `/:idRecette/add/ingredient/:idIngredient`,
  (req, res) => {
