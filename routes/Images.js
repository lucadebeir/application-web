...
image.post("/uploads", upload.single("file"), async (req, res, next) => {
  try {
    ...
    db.sequelize
      .query(
        "INSERT INTO `images`(`idImage`, `lienImage`, `nameImage`) VALUES (NULL,?,?)",
        {
          replacements: [imageUrl, req.file.originalname],
...
image.get("/image/:id", (req, res) => {
  db.sequelize
    .query("SELECT * FROM images WHERE images.idImage=?", {
...
image.get("/commentaire/:idCommentaire", (req, res) => {
  db.sequelize
    .query(
      "SELECT images.* FROM images, illustrerCommentaires, commentaires WHERE commentaires.idCommentaire = illustrerCommentaires.idCommentaire AND illustrerCommentaires.idImage = images.idImage AND commentaires.idCommentaire=?",
      {
...
image.post("/update", (req, res) => {
  db.sequelize
    .query("UPDATE illustrerRecettes SET idImage = ? WHERE idRecette = ?", {
...
image.post("/commentaires/addImage", (req, res) => {
  db.sequelize
    .query(
      "INSERT INTO `illustrerCommentaires` (`idCommentaire`, `idImage`) VALUES (?, ?)",
      {
