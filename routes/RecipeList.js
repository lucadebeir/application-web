...
recipeList.post("/add", (req, res) => {
  db.sequelize
    .query(
      "INSERT INTO `recipeLists` (`idRecipeList`, `nomRecette`, `pseudoUser`, `idRecette`) VALUES (NULL, ?, ?, ?)",
      {
        replacements: [
          req.sanitize(req.body.nomRecette),
          req.sanitize(req.body.pseudoUser),
          req.sanitize(req.body.idRecette),
        ],
...
recipeList.post("/update", (req, res) => {
  db.sequelize
    .query("UPDATE recipeLists SET complet = ? WHERE idRecipeList = ?", {
