...
menu.post('/petitDej/update', (req, res) => {
    db.sequelize.query("UPDATE menus SET idRecette = ?  WHERE idMenu = 1", {
...
menu.post('/repas/update', (req, res) => {
    db.sequelize.query("UPDATE menus SET idRecette = ?  WHERE idMenu = 2", {
            replacements: [req.sanitize(req.body.idRecette)],
            type: sequelize.QueryTypes.UPDATE
        })
        .then(() => {
...
menu.post('/douceur/update', (req, res) => {
    db.sequelize.query("UPDATE menus SET idRecette = ?  WHERE idMenu = 3", {
            replacements: [req.sanitize(req.body.idRecette)],
            type: sequelize.QueryTypes.UPDATE
        })
        .then(() => {
