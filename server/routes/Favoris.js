  ...
  Favoris.findOne({
    ...
    .then((favoris) => {
      if (!favoris) {
        ...
        db.sequelize
          .query("INSERT INTO `favoris`(`pseudo`, `idRecette`) VALUES (?, ?)", {
...
//récupérer les favoris de l'utilisateur selon nbVues
favori.get("/recipe/nbVues/:pseudo", (req, res) => {
  db.sequelize
    .query(
      "SELECT recettes.* FROM recettes INNER JOIN favoris WHERE recettes.idRecette = favoris.idRecette AND favoris.pseudo = ? ORDER BY recettes.nbVues DESC",
      {
        replacements: [req.params.pseudo],
        type: sequelize.QueryTypes.SELECT,
      }
...
//récupérer les favoris de l'utilisateur selon une catégorie et nbVues
favori.get("/recipe/nbVues/:pseudo/:idCategorie", (req, res) => {
  db.sequelize
    .query(
      "SELECT recettes.* FROM recettes INNER JOIN favoris INNER JOIN classerDans WHERE recettes.idRecette = favoris.idRecette AND recettes.idRecette = classerDans.idRecette AND classerDans.idCategorie = ? AND favoris.pseudo = ? ORDER BY recettes.nbVues DESC",
      {
        replacements: [req.params.idCategorie, req.params.pseudo],
        type: sequelize.QueryTypes.SELECT,
      }
